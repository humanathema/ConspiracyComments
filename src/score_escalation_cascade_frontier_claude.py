"""score_escalation_cascade_frontier_claude.py

Sends 136 escalation candidates to Claude API for hostile/endorsement re-scoring.
Uses Anthropic SDK instead of Vertex AI.

Input: escalation_candidates_for_frontier.csv
       stance_classifier_training_data_round7_bigval_split.parquet (for text)
...
def get_client():
    if not hasattr(_thread_local, "client"):
        ...
        _thread_local.client = Anthropic(api_key=api_key)
...
def score_one(row_idx, text, entity):
    ...
    for attempt in range(3):
        try:
            ...
            score = parse_response(resp.content[0].text)
            break
...
def main():
    ...
    # Candidates are in the same order as val_data
    escalation_data = []
    ...
    print(f"\nScoring {len(todo)} remaining rows via Claude API...", flush=True)
