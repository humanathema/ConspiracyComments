"""score_fp_v9_fullcorpus.py

Scores v9 (fp_detector_v9_blindspot) over the full-corpus stanced-predicted
subset (270,951 rows) from the blended 5-model ensemble + binconf pipeline.
Direct sibling of score_fp_v8_fullcorpus.py -- same architecture, just the
v9 checkpoint and score column name. Reconstructed here from the /tmp copy
pushed to the VM (run in parallel with v8 across both GCP projects) for
the repo's own record.

Input: fullcorpus_stanced_for_fpdetector.parquet (id, text,
  target_entity, confidence, final_label)
Output: fp_v9_fullcorpus_scores.parquet (+ fp_v9_score column)
"""
import torch
...
OUT_PATH = "/home/nash/fp_v9_fullcorpus_scores.parquet"
