"""train_fp_detector_v7_expanded.py

Round 7: third generation of the active-learning loop. Same text-only,
no-tag architecture as v5/v5b/v6, adds the 1,637 confirmed-positive and
1,120 confirmed-negative rows found by verifying v6's own full-scan flags
(2,819 rows) via blind frontier judge (see
fp_detector_v6_flagged_frontier_verified in data/experiment_log.jsonl).

Data now spans three generations:
  Gen 1 (human-verified): 76 orig + 76 backtranslation-paraphrase + 43
    val-paraphrase positives, weight 1.0.
  Gen 2 (verifying binconf's raw disagreement pool, from before v5
    existed): 149 positives / 236 negatives, weight 0.6.
  Gen 3 (verifying v5's full-scan flags): 564 positives / 714 negatives,
    weight 0.6.
  Gen 4 (verifying v6's full-scan flags, NEW this round): 1,637
    positives / 1,120 negatives, weight 0.6.
  Plus up to 1200 human-verified confidently-correct-stanced negatives,
  weight 1.0.

Same class-balance fix as v5b/v6 (positives weight multiplied by
negatives/positives ratio).
"""
import os
...
SAVE_DIR = os.environ.get("SAVE_DIR", "/home/nash/retrain_twostage/fp_detector_v7_expanded")
...
def main():
    ...
    frontier_pos_g2 = pd.read_csv("/home/nash/v5_frontier_pos.csv")
    frontier_pos_g2["weight"] = 0.6

    verified_g3 = pd.read_csv("/home/nash/fp_v5_flagged_frontier_VERIFIED.csv")
    frontier_pos_g3 = verified_g3[verified_g3["confirmed_other"] == True][["text", "target_entity"]].copy()
    frontier_pos_g3["weight"] = 0.6

    verified_g4 = pd.read_csv("/home/nash/fp_v6_flagged_frontier_VERIFIED.csv")
    frontier_pos_g4 = verified_g4[verified_g4["confirmed_other"] == True][["text", "target_entity"]].copy()
    frontier_pos_g4["weight"] = 0.6

    positives = pd.concat([orig_pos, para_pos, val_pos, frontier_pos_g2, frontier_pos_g3, frontier_pos_g4], ignore_index=True)
    positives["y"] = 1
    print(f"Positives: {len(orig_pos)} orig + {len(para_pos)} paraphrase + {len(val_pos)} val-paraphrase "
          f"+ {len(frontier_pos_g2)} g2 + {len(frontier_pos_g3)} g3 + {len(frontier_pos_g4)} g4(NEW) = {len(positives)}", flush=True)
    ...
    frontier_neg_g2 = pd.read_csv("/home/nash/v5_frontier_neg.csv")
    frontier_neg_g2["weight"] = 0.6

    frontier_neg_g3 = verified_g3[verified_g3["confirmed_still_stanced"] == True][["text", "target_entity"]].copy()
    frontier_neg_g3["weight"] = 0.6

    frontier_neg_g4 = verified_g4[verified_g4["confirmed_still_stanced"] == True][["text", "target_entity"]].copy()
    frontier_neg_g4["weight"] = 0.6

    negatives = pd.concat([bigneg, frontier_neg_g2, frontier_neg_g3, frontier_neg_g4], ignore_index=True)
    negatives["y"] = 0
    print(f"Negatives: {len(bigneg)} human-verified-pool + {len(frontier_neg_g2)} g2 "
          f"+ {len(frontier_neg_g3)} g3 + {len(frontier_neg_g4)} g4(NEW) = {len(negatives)}", flush=True)
