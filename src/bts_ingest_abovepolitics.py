#!/usr/bin/env python
"""bts_ingest_abovepolitics.py

Wayback Machine ingestion pipeline for AbovePolitics.com, the third board
in "The Above Network" family alongside ATS and BelowTopSecret (see
handoff/bts_ingestion_scoping_findings.md). Structurally identical to
bts_ingest_archive.py - same dual-URL-scheme situation (pre-2007
`/thread<ID>/pg<N>`, 2007+ `/forum/thread<ID>/pg<N>`), same reused
download/parse machinery from src/ingest_ats_archive.py. Kept as a
separate script (not a --site flag on the BTS one) rather than sharing a
module, since AbovePolitics' archive is small (~9k pages) and duplicating
~150 lines here is simpler to reason about than adding an abstraction
layer for a one-time ingestion job.

Named with the bts_ prefix (not ap_) deliberately, to keep this whole
supplemental-corpus effort grouped under one filename prefix per the
task file's collision-avoidance guardrail, rather than introducing a
second new prefix.
...
DEFAULT_CACHE_DIR = "data/raw/bts_abovepolitics_raw_html"
DEFAULT_OUTPUT_FILE = "data/processed/bts_abovepolitics_comments.jsonl"
DEFAULT_METADATA_FILE = "data/processed/bts_abovepolitics_metadata.json"
PARSED_LEDGER_NAME = "bts_abovepolitics_parsed_files.txt"

THREAD_REGEX = re.compile(r'(?:/forum)?/thread(\d+)/pg(\d+|lastpost)?', re.IGNORECASE)

CDX_URL_PREFIXES = [
    "abovepolitics.com/forum/thread",
    "abovepolitics.com/thread",
]
...
def parse_and_export_directory(cache_dir=DEFAULT_CACHE_DIR, output_path=DEFAULT_OUTPUT_FILE):
    """See bts_ingest_archive.parse_and_export_directory's docstring for
    why this is a copy (site-specific ledger filename) rather than an
    import."""
...
def main():
    parser = argparse.ArgumentParser(description="AbovePolitics.com Wayback Machine Ingestion Pipeline")
