...
Outputs a centralized long-format cache:
  - data/processed/entity_mentions_cache_short.parquet
  - data/processed/entity_mentions_cache_short.csv

Columns:
  comment_id | entity_key | construct | p_hostile | p_endorsement | p_other | predicted_label | is_list_dump | population
"""
import os
import re
import sys
...
from rerun_refined_regressions_v2 import (
    ...
    THREAD_PATH, BRIGADE_PATH, VERIFIED_MAVERICK_ADDITIONS,
)
...
OUT_CSV = 'data/processed/entity_mentions_cache_short.csv'
...
def main():
    print("=== Materializing Centralized Long-Format Short-Comment Entity-Mentions Cache ===")
    ...
    con = duckdb.connect()
    print(f"\n[Index Pass] Scanning short comments from {SHORT_COMMENTS_PATH}...")
    # Clean filters applied: exclude brigaded or high-crosspost threads to match long comment population exactly.
    query_unfiltered = f"""
        SELECT
            s.id, s.text,
            CAST(regexp_matches(s.text, $1) AS INTEGER) as has_maverick_regex,
            CAST(regexp_matches(s.text, $2) AS INTEGER) as has_consensus_expert_regex
    ...
    """
    df_short_unf = con.execute(query_unfiltered, ["(?i)" + rx_mav.pattern, "(?i)" + rx_con.pattern]).df()
    print(f"  Loaded {len(df_short_unf):,} matching comments.")

    print("  Applying fallback disambiguation lookup...")
    resolved_mav = df_short_unf["id"].astype(str).map(lookup)
    df_short_unf["has_maverick"] = (df_short_unf["has_maverick_regex"].astype(bool) | resolved_mav.isin(VALID_MAVERICK_CANDIDATES)).astype(int)
    resolved_con = df_short_unf["id"].astype(str).map(consensus_lookup)
    df_short_unf["has_consensus_expert"] = (df_short_unf["has_consensus_expert_regex"].astype(bool) | resolved_con.isin(VALID_CONSENSUS_CANDIDATES)).astype(int)
    df_short_unf = df_short_unf.drop(columns=["has_maverick_regex", "has_consensus_expert_regex"])

    # Keep only comments with at least one mention of interest
    short_mentions = df_short_unf[(df_short_unf['has_maverick'] == 1) | (df_short_unf['has_consensus_expert'] == 1)].copy()
    print(f"  Found {len(short_mentions):,} comments with mentions in short comments.")

    short_mentions['id'] = short_mentions['id'].astype(str)
    
    rows_to_score = []
    
    print("\nExtracting windows across all granularities...")
    for idx, row in short_mentions.iterrows():
        cid = row['id']
        text = str(row['text'])
